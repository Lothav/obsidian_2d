...
#include "Obsidian2D/Renderer/Vulkan/Instance.h"
    ...
    namespace Core
    {
        ...
        public:
            ...
            int start(Obsidian2D::Core::App* app)
            {
                ...
                Obsidian2D::Renderer::Vulkan::Instance* renderer = nullptr;
                
                if(config.renderer == "default") {
                    renderer = new Obsidian2D::Renderer::Vulkan::Instance();
                }

                /* Init Renderer, audio, Resource manager, AI, Input based on App Config*/

                renderer->registerLogCallback([](const std::string& line) {
                    Obsidian2D::Util::Logger::write("[Obsidian2D::Renderer::Vulkan] " + line);
                });

                VkResult res = renderer->init();
                if (res == VK_ERROR_INCOMPATIBLE_DRIVER) {
                    this->log("Cannot find a compatible Vulkan ICD");
                    return Obsidian2D::Error::RendererInit;
                } else if(res != 0){
                    this->log("Unknow Vulkan Error");
                    return Obsidian2D::Error::RendererInit;
                }
